...
Note.prototype={
	...
	createNote: function(){
		...
		if(!this.id) this.$note.css('bottom','10px');//新增的放在右边
	},
	setStyle: function(){
	...
	bindEvent: function(){
		...
		//conteteditable没有change事件，所以这里做了模拟通过判断元素内容改动，执行save
		...
		$noteHead.on('mousedown',function(e){
			var evtX = e.pageX-$note.offset().left, //evtX计算事件的触发点在dialog内部到dialog的左边缘的距离
			 evtY = e.pageY-$note.offset().top;
			$note.addClass('draggable').data('evtPos',{x:evtX,y:evtY});
			console.log($note)//把事件到dialog边缘的距离保存下来
		}).on('mouseup',function(){
